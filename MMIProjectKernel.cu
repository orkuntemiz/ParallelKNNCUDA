...
float compute_distance(const float* ref,
    ...
    for (int d = 0; d < dim; ++d) {
        const float diff = 
            ref[d * ref_num + ref_index] - query[d * query_num + query_index]; // Locate the location of the query and reference points for each dimension
        sum += diff * diff;
    }
...
int main(void) {
    ...
    test(ref, ref_num, query, query_num, dim, k, knn_dist, knn_index, &knn_cuda_global, "knn_cuda_global", 10);
    test(ref, ref_num, query, query_num, dim, k, knn_dist, knn_index, &knn_cuda_texture, "knn_cuda_texture", 10);
